#!/usr/bin/env python3
"""
🧙‍♂️ WizardCoder-15B NewTek Enterprise Training - FIXED VERSION
Correct target modules and BitsAndBytesConfig for proper GPU usage
"""
...
def setup_wizardcoder_model():
    """Setup WizardCoder-15B with CORRECT target modules and proper quantization"""
    log("🧙‍♂️ Loading WizardCoder-15B model with FIXED configuration...")
    
    model_name = "WizardLM/WizardCoder-15B-V1.0"
    
    # Proper quantization config (replaces deprecated load_in_8bit)
    ...
    # CORRECTED target modules for WizardCoder-15B
    # These are the actual module names in the WizardCoder architecture
    ...
    log("✅ WizardCoder-15B setup complete with FIXED target modules")
    return model, tokenizer

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--output_dir", default="/workspace/newtek-wizardcoder-15b-FIXED")
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--learning_rate", type=float, default=2e-4)
    args = parser.parse_args()
    
    log("🧙‍♂️ Starting NewTek WizardCoder-15B Training - FIXED VERSION...")
    ...
    if torch.cuda.is_available():
        ...
        log(f"🎯 GPU Name: {torch.cuda.get_device_properties(0).name}")
    
    # Load your massive training dataset
    dataset = load_all_training_data()
    log(f"📊 Training on {len(dataset)} enterprise examples")
    
    # Setup WizardCoder model with FIXED configuration
    model, tokenizer = setup_wizardcoder_model()
    
    # Training arguments optimized for RTX A6000 with proper memory management
    training_args = TrainingArguments(
        output_dir=args.output_dir,
        num_train_epochs=args.epochs,
        per_device_train_batch_size=args.batch_size,
        gradient_accumulation_steps=16,  # Large for memory efficiency
        warmup_steps=100,
    ...
    log(f"💰 Cost: ~${6 * 0.49:.2f} for 6 hours")
    ...
    try:
        ...
        log("💡 Try reducing batch size or gradient accumulation steps")
        raise
...
Response:"""
    ...
    log("💡 Model uses CORRECT target modules and proper quantization")
